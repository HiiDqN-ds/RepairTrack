...
# ---------------------------------------------------------------------------
# Per-seller assistant permissions — an admin can grant/revoke which AI
# assistant functions (tools) an individual seller account is allowed to
# use (see SELLER_ALLOWED_TOOLS / _execute_assistant_tool in app.py).
# Stored as a JSON array of tool names in users.assistant_tools. NULL means
# "not yet configured by an admin" and falls back to
# DEFAULT_SELLER_ASSISTANT_TOOLS, so existing sellers keep working exactly
# as before until an admin explicitly saves a selection on their edit page.
# ---------------------------------------------------------------------------

DEFAULT_SELLER_ASSISTANT_TOOLS = {
    'get_summary', 'list_items', 'list_low_stock', 'list_recent_sales',
    'list_recent_orders', 'quick_sell', 'print_barcode', 'remember_note',
}


def ensure_users_assistant_tools_column() -> None:
    """Add users.assistant_tools if an older DB predates the per-seller
    assistant permission feature. Same safe/idempotent pattern as
    ensure_products_sku_column(): only issues DDL when actually missing.
    """
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users'
                  AND COLUMN_NAME = 'assistant_tools';
                """
            )
            if not cur.fetchone():
                cur.execute("ALTER TABLE users ADD COLUMN assistant_tools TEXT NULL;")
            conn.commit()
    except Exception:
        conn.rollback()
        raise
...
def get_seller_allowed_tools(username):
    """Set of assistant tool names this seller may use. Falls back to
    DEFAULT_SELLER_ASSISTANT_TOOLS if an admin hasn't explicitly configured
    this seller yet (assistant_tools column is NULL)."""
    user = find_user(username)
    if not user or user.get('assistant_tools') is None:
        return set(DEFAULT_SELLER_ASSISTANT_TOOLS)
    try:
        stored = json.loads(user['assistant_tools'])
...
def set_seller_allowed_tools(username, tool_names):
    """Admin grants/revokes assistant functions for one seller account."""
    execute_query(
        "UPDATE users SET assistant_tools = %s WHERE username = %s;",
        (json.dumps(sorted(set(tool_names))), username),
    )

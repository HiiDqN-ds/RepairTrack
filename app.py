...
def _enforce_shop_feature_flags():
    ...
    if path.startswith('/admin/items/barcode_print') and not shop_feature_enabled('barcode_printing'):
...
# Every debt gets its own short random reference number (independent from
...
# Per-seller assistant permissions (an admin grants/revokes individual AI
# assistant functions per seller account) — make sure the column exists
# even on a database created before this feature.
try:
    ensure_users_assistant_tools_column()
except Exception:
    logger.exception('Could not ensure users.assistant_tools column at startup')
...
def edit_seller(username):
    ...
    if request.method == 'POST':
        ...
        # Assistant (AI) function permissions for this seller — checkboxes
        # are only rendered/submitted for names in ALL_ASSISTANT_TOOL_NAMES,
        # so this is always a subset of the real tool catalog.
        selected_tools = [t for t in request.form.getlist('assistant_tools') if t in ALL_ASSISTANT_TOOL_NAMES]
        set_seller_allowed_tools(username, selected_tools)
    ...
    seller_tools = get_seller_allowed_tools(username)
    assistant_tool_catalog = sorted(ASSISTANT_TOOLS, key=lambda t: t['name'])
    return render_template(
        'edit_seller.html', seller=seller, is_self=is_self,
        assistant_tool_catalog=assistant_tool_catalog, seller_tools=seller_tools,
    )
...
def add_item():
    if request.method == 'POST':
        ...
        insert_query = """
            ...
            (product_name, description, quantity, barcode, sku, purchase_price, selling_price, min_selling_price, date_added)
...
def _add_item_record(product_name, quantity=0, purchase_price=0, selling_price=0, min_selling_price=None,
                      barcode_value=None, sku=None, description=''):
    ...
    execute_query(
        ...
        (name, description or '', int(quantity or 0), barcode_value, sku,
         float(purchase_price or 0), float(selling_price or 0), min_selling_price, datetime.now().strftime('%Y-%m-%d')),
    )
...
# The chat is available to sellers too (not just admins), but the tools
# themselves were written assuming an admin caller and have no per-row
# ownership checks. Rather than silently let a seller pay salaries or wipe
# out debts through the chat, each seller account is only allowed the
# subset of tools an admin has explicitly granted it (see
# get_seller_allowed_tools()/set_seller_allowed_tools() in db.py, and the
# checkbox list on the seller edit page). ALL_ASSISTANT_TOOL_NAMES is the
# full catalog an admin can grant from; DEFAULT_SELLER_ASSISTANT_TOOLS
# (db.py) is what a seller gets until an admin explicitly configures them.
ALL_ASSISTANT_TOOL_NAMES = {t['name'] for t in ASSISTANT_TOOLS}

# Backwards-compatible alias: some code/comments still refer to this name.
SELLER_ALLOWED_TOOLS = DEFAULT_SELLER_ASSISTANT_TOOLS


def _assistant_tools_for_session(username):
    """The set of tool names the current session may call: everything for
    an admin, or this specific seller's admin-granted subset."""
    if session.get('role') == 'admin':
        return set(ALL_ASSISTANT_TOOL_NAMES)
    return get_seller_allowed_tools(username)
...
def _execute_assistant_tool(name, args, username):
    if name not in _assistant_tools_for_session(username):
        if session.get('role') == 'admin':
            raise PermissionError(f"'{name}' is not a recognized tool.")
        raise PermissionError(f"'{name}' has not been enabled for your account. Ask an admin to grant it.")
    if name == 'get_summary':
        today = datetime.now().date()
        items = load_items()
        debts_open = fetch_one("SELECT COALESCE(SUM(amount), 0) AS total, COUNT(*) AS cnt FROM debts WHERE paid = FALSE;")
...
def _run_assistant_chat(user_message, history, username, lang=None):
    ...
    # Only offer the model tools this caller is actually allowed to run —
    # keeps a restricted seller's assistant from even attempting (and then
    # being blocked on) something it has no permission for.
    allowed_tool_names = _assistant_tools_for_session(username)
    available_tools = [t for t in ASSISTANT_TOOLS if t['name'] in allowed_tool_names]
...
def assistant_app():
    ...
    enforced in /assistant/api/chat via each seller's admin-granted tool
    list — a seller installing this doesn't get more access than they had
    in the dashboard's chat widget, just the same assistant full-screen with
    its own icon). Installed via the browser's "Add to Home Screen" (Android:
